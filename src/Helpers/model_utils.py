...
def load_hf_model_or_local(
    ...
    **kwargs
    ...
    try:
        ...
        if not is_tokenizer:
            print(f"[Fallback] Initializing {model_name} from config: {e}")
            from transformers import AutoConfig
            cfg = AutoConfig.from_pretrained(model_name, cache_dir=str(local_dir))
            instance = loader.from_config(cfg)
            return instance
        raise
