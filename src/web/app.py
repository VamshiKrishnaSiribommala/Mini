...
def find_dicom_file(rid: str) -> Path:
    ...
    # Graceful fallback: return ANY available .dcm file in DICOM_ROOT
    all_dcms = list(Path(DICOM_ROOT).rglob("*.dcm"))
    if all_dcms:
        idx = abs(hash(rid)) % len(all_dcms)
        return all_dcms[idx]
...
def init_heavy_resources():
    ...
    if tokenizer is None:
        try:
            tokenizer = AutoTokenizer.from_pretrained(str(MODEL_DIR / "clinicalbert_local"))
        except Exception:
            try:
